...
Q:  ![[Pasted image 20251014091550.png]]
A: **Правильный ответ:** в) верно; a), б), г) — неверны.
	
**Почему:** при оценке обычной линейной регрессии по МНК справедливо равенство
	
$\tilde\beta_0=\bar y-\tilde\beta_1\bar x.$
	
Отсюда при $X=\bar x$ предсказанное значение
	
$\tilde y=\tilde\beta_0+\tilde\beta_1\bar x=\bar y.$.
	
То есть **линия регрессии проходит через точку $(\bar x,\bar y)$**, следовательно предсказание в точке $\bar x$ равно $\bar y$​ (вариант c).
	
---
	
Разбор остальных вариантов:
	
- a) Наблюдаемое $y_i$​ при $x_i=\bar x$ в общем случае **не равно** $\bar y$​ — отдельные наблюдения могут отличаться от среднего.
    
- б) Формулировка «в среднем равно $\bar y$, но не обязательно в каждом конкретном случае» некорректна: предсказание при $X=\bar x$ равно $\bar y$​ **в каждом таком случае** (не только «в среднем»).  
    (Если есть несколько наблюдений с $x_i=\bar x$, их предсказания все будут равны $\bar y$​.)
    
- г) Отклонение одного конкретного наблюдения $e_i=y_i-\tilde y_i$​ при $x_i=\tilde x$ **не обязано быть минимальным**: МНК минимизирует **сумму квадратов** остатков по всем точкам, а не абсолютную величину остатка для конкретной точки.
    
	
Коротко: регрессионная прямая проходит через центр масс данных $(\bar x,\bar y)$, поэтому предсказание в $\bar x$ равно $\bar y$.
	
**TL;DR:**
Короче, первое не подходит, т.к. там речь идёт о наблюдаемом значении $y$, т.е. значении из выборки, а не том, которое мы получили из нашей модели.
<!--ID: 1760422899983-->
